instances = [
    ...
    ("class_weight", 'weights will be multiplied with sample_weight if sample_weight is specified'),
    ("alpha",'used to compute the learning rate when learning_rate is set to "optimal"'),
    ...
    ("n_components",'If svd_solver == "arpack", the number of components must be strictly less than the minimum of n_features and n_samples'),
    ...
    ("digits", 'if output_dict is True, this will be ignored and the returned values will not be rounded'),
    ("labels", 'The set of labels to include when average != "binary", and their order if average is None'),
    ("pos_label", 'The class to report if average="binary" and the data is binary'),
    ("average", 'if “binary”, then only report results for the class specified by pos_label'),
    ("Y_score", 'in the multiclass case, the order of the class scores must correspond to the order of labels, if provided, or else to the numerical or lexicographical order of the labels in y_true'),
    ("average", 'Will be ignored when y_true is binary'),
    ("labels", 'If None, the numerical or lexicographical order of the labels in y_true is used'),
    ("estimator", 'Either estimator needs to provide a score function, or scoring must be passed'),
    ...
    ("train_size", 'If None, the value is automatically set to the complement of the test_size'),
