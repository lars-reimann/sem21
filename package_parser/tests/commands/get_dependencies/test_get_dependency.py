...
from package_parser.commands.get_api._model import (
    Action, ParameterIsIgnored, ParameterIsIllegal, Condition, ParameterIsNone, ParameterHasValue
)
from package_parser.commands.get_dependencies._get_dependency import (
    extract_lefts_and_rights, extract_action, extract_condition
)
...
def test_extract_action():
    ...
    action_is_illegal = nlp("Individual weights for each sample raises error if sample_weight is passed and base_estimator fit method does not support it")
    ...
    assert illegal_action == ParameterIsIllegal(action="Individual weights for each sample raises error")
